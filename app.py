...
API_URL = "https://api-inference.huggingface.co/models/distilgpt2"  # free working model
...
def query_hf(prompt):
    ...
    if response.status_code == 200:
        ...
        if isinstance(data, list):
            return data[0].get('generated_text', "No response found.")
        return str(data)
    else:
...
if st.button("Save Medication"):
    if med and dose:
        ...
        st.error("Enter both medication and dose.")
...
if st.button("Log Symptom"):
    if symptom:
        ...
        st.error("Enter a symptom or vital.")
...
if st.button("Ask"):
    if not HF_API_KEY:
        st.error("No Hugging Face API key found in secrets.")
    elif not question:
        st.error("Type a question to ask MedPal.")
    else:
